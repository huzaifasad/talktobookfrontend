...
export default function ChatGpt() {
  ...
  const updatequestionnumber = async () => {
    ...
    try {
      const response = await fetch("http://localhost:529/add/question/info", {
        method: "POST",
        headers: {
  ...
  const addBookData = async () => {
    ...
    if (bookCount <= uplanballow || uplanballow === "unlimited") {
      ...
      try {
        const response = await fetch("http://localhost:529/add/book/info", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  const googleSignIn = async () => {
    try {
      ...
      const response = await fetch("http://localhost:529/create", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handlesend = async () => {
    ...
    try {
      ...
      if (selectedBook) {
        ...
        const response = await fetch("http://localhost:529/api/chat", {
          method: "post",
          headers: {
            "Content-Type": "application/json",
            Authorization:
  ...
  const readdata = async (id) => {
    try {
      let result = await fetch(`http://localhost:529/read/book/info/${id}`);
  ...
  const allreaddata = async (id) => {
    try {
      let result = await fetch(`http://localhost:529/read/all/info/${id}`);
      result = await result.json();
  ...
  const deletebooks = async (title) => {
    ...
    let result = await fetch(
      `http://localhost:529/delete/book/${xuserid}/${title}`,
      {
        method: "delete",
      }
    );
  ...
  return (
    <div className="h-screen w-screen bg-[#181a1d] flex">
      <div className="w-[20%]  bg-[#1d262d] h-screen text-yellow-50">
    ...
    {uplan === "genius" || uplan === "premium" && (
      <>
        <h2>You have unlimited questions</h2>
                    ...
                    <div
                      className=" fixed w-64 ml-1 mr-1 bg-green-600  rounded-xl"
                      style={{
                        zIndex: 100,
                        bottom: "80px",
                        left: "3px",
                    ...
                    >
                      {users && uplan ? (
                        ...
                        <p className="bg-blue-500 hover:bg-blue-700 text-white w-full rounded h-10">
                         <Link to='/price'><h1>Upgrade</h1></Link> 
                        </p>
                      )}
                    </div>
                  </div>
      ...
      <div className="w-[80%] bg-[#2a3742] text-white">
